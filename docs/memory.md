...
- **SessionStart** may append a short pointer (at most about 350 tokens): a
  graph one-liner, or N-memories plus a recall command. Empty store stays
  silent except a pending distill line. Pending harvest or distill may add one
  extra line. OPEN harvest review is not injected.
...
Bodies stay behind `get`. See [configuration.md](configuration.md).

Optional denser embeddings need Python 3 and a model under
`~/.superopen/models/`. If Python is missing, search still works with a
hash fallback. See [troubleshooting.md](troubleshooting.md).
