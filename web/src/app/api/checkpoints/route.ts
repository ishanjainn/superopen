...
/** Restore a checkpoint created on git commit / session finalize. Manual create was removed. */
  ...
  return runWithProjectAsync(project, async () => {
    if (op === "create") {
      return NextResponse.json(
        {
          error:
            "Manual checkpoints were removed. Snapshots are created automatically on git commit and session finalize.",
        },
        { status: 400 }
      );
    }
    if (op === "restore") {
      ...
      const res = await soJSON(["checkpoint", "restore", sessionId, checkpointId], {
        cwd: repoCwd(),
        timeoutMs: 60_000,
      });
      if (res.ok === false) {
