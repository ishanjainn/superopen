...
func WriteUpgradeBrief(paths harness.Paths, p discover.Profile) error {
	...
	b.WriteString("Coding agents: use **your own model** to upgrade `.so/` context/guardrails/evals.\n")
	b.WriteString("Do **not** ask the user for an API key. Produce the JSON below, then run:\n\n")
	b.WriteString("```bash\nso apply-upgrade\n```\n\n")
	b.WriteString("(paste/write the JSON to stdin, or `so apply-upgrade path/to.json`)\n\n")
...
func applyLLMUpgrade(paths harness.Paths, p discover.Profile, out llmHarnessOut) error {
	header := "<!-- Generated by so init (LLM upgrade). Edit freely; so sync will not overwrite. -->\n\n"
	if strings.TrimSpace(out.ArchitectureMD) != "" {
		if err := os.WriteFile(filepath.Join(paths.KnowledgeDir, "architecture.md"), []byte(header+strings.TrimSpace(out.ArchitectureMD)+"\n"), 0o644); err != nil {
			return err
		}
	}
	if strings.TrimSpace(out.ConventionsMD) != "" {
		if err := os.WriteFile(filepath.Join(paths.KnowledgeDir, "conventions.md"), []byte(header+strings.TrimSpace(out.ConventionsMD)+"\n"), 0o644); err != nil {
			return err
		}
	}
...
Prefer .so/ context before raw exploration.
- Graph: .so/graph/graph.json
