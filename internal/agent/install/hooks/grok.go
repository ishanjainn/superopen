...
const (
	grokManagedHookFileName = "superopen-endpoint.json"
	grokManagedHookMarker   = "superopen-managed-grok-hooks:v1"
)
...
func installGrokHooks(path, binaryPath, logPath, configPath string) error {
	...
	hooks := grokHooksFile{
		Description: "Superopen managed Grok endpoint telemetry hooks.",
		Superopen:   grokManagedHookMarker,
		Hooks: map[string][]grokHookGroup{
			"SessionStart":     {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "session-start")}}}},
			"UserPromptSubmit": {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "prompt-submit"), Timeout: 30}}}},
			"PreToolUse":       {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "pre-tool"), Timeout: 10}}}},
			"PostToolUse":      {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "post-tool"), Timeout: 10}}}},
			"PostToolUseFailure": {
				{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "post-tool"), Timeout: 10}}},
			},
			"Stop":       {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "stop"), Timeout: 45}}}},
			"SessionEnd": {{Hooks: []grokHookRef{{Type: "command", Command: hookLine(prefix, "session-end")}}}},
		},
	}
