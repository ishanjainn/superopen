...
func installVSCodeHooks(path, binaryPath, logPath, configPath string) error {
	...
	endpointHooks := map[string]vscodeHookRef{
		"SessionStart":     {Type: "command", Command: hookLine(prefix, "session-start")},
		"UserPromptSubmit": {Type: "command", Command: hookLine(prefix, "prompt-submit"), Timeout: 30},
		"PreToolUse":       {Type: "command", Command: hookLine(prefix, "pre-tool")},
		"PostToolUse":      {Type: "command", Command: hookLine(prefix, "post-tool")},
		"SubagentStart":    {Type: "command", Command: hookLine(prefix, "subagent-start")},
		"SubagentStop":     {Type: "command", Command: hookLine(prefix, "subagent-stop")},
		"Stop":             {Type: "command", Command: hookLine(prefix, "stop"), Timeout: 45},
	}
...
func vscodeHooksPath(level Level) (string, error) {
	...
	case LevelProject:
		...
		return filepath.Join(cwd, ".github", "hooks", "superopen.json"), nil
	default:
