// Cursor-specific install path: merge Superopen hook entries into the
// user-scope ~/.cursor/hooks.json file instead of dropping a separate
// plugin tree under ~/.cursor/plugins/superopen/.
//
...
func installCursorHooks(dryRun bool) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("locate so binary: %w (install so and ensure it is on PATH)", err)
	}
	...
	// rewrite the bare `so ...` commands to use the absolute
	...
	tmplBytes = patchManifestBytes("hooks/hooks.json", tmplBytes, soBin)
...
func isOurHookEntry(raw json.RawMessage) bool {
	var entry struct {
