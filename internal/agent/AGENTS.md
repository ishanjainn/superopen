...
| **Silent lifecycle** | SessionEnd / sessionEnd detach `so sessions finalize` (no extra model text). Prompt-submit is silent unless the prompt is prior-work/personal (or the workspace is a memory store with little source); then inject matching recalled bodies, or the pointer-only index if recall is empty. PostToolUse / Stop stay silent. SessionStart is one route only: graph one-liner when the repo has source, or N-memories + recall command when live diary rows exist and there is little/no source. Harvest stays on-demand (`references/harvest.md`), not SessionStart. Only explore-tool nudges (and SubagentStart where the host has subagents). Skill / `AGENTS.md` / `SKILL.md` Reads are not “skipped graph”. |
