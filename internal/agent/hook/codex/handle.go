...
func handle(ctx context.Context, in normalize.Input) error {
	...
	case "Stop":
		...
		// transcript without closing the overall coding session. Explicit
...
func primeSubagentLink(p codexPayload) {
	...
	st.ParentConversationID = meta.ParentSessionID
	st.IsBackgroundAgent = true
	sessionstate.Save(p.SessionID, "codex", st)
	_ = agentlinks.Register(agentlinks.SessionsDir(p.Cwd), p.SessionID, meta.ParentSessionID, "codex", "codex-session-meta")
}
