...
import (
	...
	"github.com/ishanjainn/superopen/internal/agent/tailfile"
...
func handle(ctx context.Context, in normalize.Input) error {
	...
	case "PreToolUse":
		...
		if p.ToolName == "Task" && p.ToolUseID != "" && p.SessionID != "" {
			...
			_ = agentlinks.NotePending(agentlinks.SessionsDir(p.CWD), p.SessionID, in.Vendor, p.ToolUseID)
		}
	...
	case "PostToolUse":
		linkClaudeTaskAgent(p, in.Vendor)
	...
	case "SubagentStop":
		...
		return in.Emit.EmitSubagent(normalize.Subagent{
			SessionID:            p.SessionID,
			ParentConversationID: p.SessionID,
			SubagentID:           p.TaskID,
...
// linkClaudeTaskAgent records agentId from Task/Agent tool_response so
// nested agent transcripts never become top-level Sessions rows.
func linkClaudeTaskAgent(p claudePayload, vendor string) {
	...
	agentID := agentlinks.ExtractAgentID(string(p.ToolResponse))
	if agentID == "" {
		return
	}
	_ = agentlinks.Register(agentlinks.SessionsDir(p.CWD), agentID, p.SessionID, vendor, "claude-task-result")
}
