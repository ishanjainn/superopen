...
import (
	...
	"github.com/ishanjainn/superopen/internal/agent/classify"
	"github.com/ishanjainn/superopen/internal/agent/detect"
...
func handle(ctx context.Context, in normalize.Input) error {
	...
	if err := json.Unmarshal(in.Payload, &p); err != nil {
		fmt.Fprintf(os.Stderr, "so: cursor payload parse failed: %v\n", err)
		return nil
	}
	event := in.Event
	if event == "" {
	...
	cls := classify.Classify(classify.Inputs{
		...
		RepoURL:              vcs.RepoURL,
		RepoAllowlist:        classify.EnvAllowlist(),
	})
	...
	case "sessionEnd":
		...
		// Drain real token totals + priced USD accumulated on each `stop`.
		// Cursor does not send USD; we price from real token counters via
		// pricing.Lookup (list rates). Never invent tokens or char-estimates.
	...
	case "stop":
		...
		if inTok, outTok, cacheRead, cacheWrite, ok := realTokenUsage(p); ok {
			...
			model := p.Model
			if model == "" {
				if st := sessionstate.Load(sessionID, in.Vendor); st != nil {
					model = st.Model
				}
			}
			if model != "" {
			...
			// Accurate list-price cost from real Cursor token counters (not estimates).
			var cost float64
			if rate := pricing.Lookup(model); rate.InputPer1M > 0 || rate.OutputPer1M > 0 {
				cost = rate.Cost(inTok, outTok, cacheRead, cacheWrite)
				if cost > 0 {
					attrs["gen_ai.usage.cost"] = cost
				}
			}
			accumulateStopTokens(sessionID, in.Vendor, inTok, outTok, cost)
		}
	...
	case "postToolUse":
		linkCursorTaskAgent(cwd, sessionID, in.Vendor, p)
	...
	case "subagentStart":
		linkCursorSubagentLifecycle(cwd, sessionID, in.Vendor, p, true)
	...
	case "subagentStop":
		linkCursorSubagentLifecycle(cwd, sessionID, in.Vendor, p, false)
		startedAt := time.Now().Add(-time.Duration(p.DurationMs) * time.Millisecond)
...
// accumulateStopTokens adds one stop's real token counts + priced USD into
// the sessionstate cache so sessionEnd can stamp session-root totals.
func accumulateStopTokens(sessionID, vendor string, inTok, outTok int64, costUSD float64) {
	if sessionID == "" {
		return
	}
...
// applyAccumulatedTokens copies stop-accumulated token totals and priced
// USD onto the session-root struct. Cost comes from pricing.Cost(real tokens),
// never from char-count estimates.
...
// linkCursorSubagentLifecycle records parent→child so Task agents with
// their own conversation_id never appear as top-level Sessions rows.
func linkCursorSubagentLifecycle(cwd, parentSessionID, vendor string, p cursorPayload, starting bool) {
	parent := strings.TrimSpace(p.ParentConversationID)
	if parent == "" {
	...
	sessionsDir := agentlinks.SessionsDir(cwd)
	if starting {
		_ = agentlinks.NotePending(sessionsDir, parent, vendor, firstNonEmpty(p.ToolCallID, p.ToolUseID))
	}
...
func linkCursorTaskAgent(cwd, parentSessionID, vendor string, p cursorPayload) {
	if !isTaskToolName(p.ToolName) || parentSessionID == "" {
		return
	}
	agentID := agentlinks.ExtractAgentID(p.ToolOutput, p.ResultJSON, p.AgentMessage, string(p.ToolInput))
	if agentID == "" {
		return
	}
	_ = agentlinks.Register(agentlinks.SessionsDir(cwd), agentID, parentSessionID, vendor, "cursor-task-result")
}
