...
func TestStripCodexSuperopenSections(t *testing.T) {
	sample := `model = "gpt-5"

[marketplaces.superopen]
source = "/Users/me/.local/share/so/codex-marketplace"
...
`
	...
	for _, needle := range []string{
		...
		if strings.Contains(out, needle) {
			t.Errorf("should strip %q\n%s", needle, out)
		}
	}
	if !strings.Contains(out, `[features]`) {
		t.Errorf("should keep features\n%s", out)
	}
}
...
func TestStripClaudeMarketplaceJSONDryRun(t *testing.T) {
	...
	path := filepath.Join(pluginDir, "known_marketplaces.json")
	mustJSONWrite(t, path, map[string]any{
		"so": map[string]any{"installLocation": "/x"},
	})
	before, _ := os.ReadFile(path)
