...
func corrEvent(ts, action, sessionID string, mut func(*Event)) Event {
	e := Event{
		Timestamp: ts,
		Event:     EventInfo{Action: action},
...
func readThenEgressRule(t *testing.T) *CompiledRule {
	...
	rule := &Rule{
		...
		Severity: SeverityHigh, Status: StatusExperimental, Posture: PostureDetect,
		Emit: Emit{Reason: "x"},
...
func withEnv(e *Event) { e.File = &FileInfo{Path: ".env"} }
func withCurl(e *Event) {
	e.Command = &CommandInfo{Command: "curl https://x"}
}

func TestCorrelationOrderedInWindow(t *testing.T) {
	c := readThenEgressRule(t)
	v, err := c.Evaluate([]Event{
