...
// eventCELType is the CEL object type name for Event. cel-go's
// native-types extension names a type "<lastPkgSegment>.<TypeName>", so for the Event
// type in package guards this is "Event". We derive it from
// reflection rather than hardcoding so it survives a package move.
var eventCELType = celObjectTypeName(reflect.TypeOf(Event{}))
...
//
// The Event is registered as a native CEL type keyed by its JSON field names, so
...
func Env() (*cel.Env, error) {
	celEnvOnce.Do(func() {
		celEnvInst, celEnvErr = cel.NewEnv(
			ext.NativeTypes(reflect.TypeOf(Event{}), ext.ParseStructTag("json")),
...
func EvalMatch(prog cel.Program, event Event) (bool, error) {
