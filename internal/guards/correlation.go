...
func (c *CompiledRule) evaluateCorrelation(events []Event) (Verdict, error) {
...
func (c *CompiledRule) matchSession(events []Event) ([]Event, error) {
...
// tries the next one.
func (c *CompiledRule) completeFrom(events []Event, start int) ([]Event, error) {
	startTime, startKnown := eventTime(events[start])
	final := len(c.steps) - 1
	stepIdx := 1
	seq := make([]Event, 1, len(c.steps))
	seq[0] = events[start]
...
func groupBySession(events []Event) (map[string][]Event, []string) {
