...
import (
	...
	"github.com/superopen/so/internal/coding/hook/claudecode"
	"github.com/superopen/so/internal/coding/hook/codex"
	"github.com/superopen/so/internal/coding/hook/cursor"
	"github.com/superopen/so/internal/coding/hook/generic"
	"github.com/superopen/so/internal/coding/hook/opencode"
	"github.com/superopen/so/internal/coding/hook/pi"
	"github.com/superopen/so/internal/coding/identity"
	"github.com/superopen/so/internal/coding/normalize"
	"github.com/superopen/so/internal/coding/sessionstate"
	"github.com/superopen/so/internal/agentconfig"
	"github.com/superopen/so/internal/codingotlp"
...
func run(cmd *cobra.Command, vendor, event string) (rerr error) {
	...
	// Optional debug tee. When SUPEROPEN_DEBUG_PAYLOAD_DIR is set we
	...
	if err := agentconfig.PromoteFileToEnv(); err != nil {
	...
	// SUPEROPEN_USER is exported), one with the OS username (the last-
	// resort fallback in resolveLocalUser).
	//
	// Priority (highest first):
	//   1. SUPEROPEN_USER env var (explicit override, usually exported
	...
	// session row and one sessionstate cache for that thread.
	sessionID := probe.SessionID
	if probe.ConversationID != "" {
		sessionID = probe.ConversationID
	}
	...
	// Fold Task/subagent child conversations under their parent chat.
	// Cursor Task agents get a fresh conversation_id with no parent
	// field; we recover the link via agent-links (Task agentId),
	// pending subagentStart claims, and Cursor's on-disk
	// agent-transcripts/<parent>/subagents/<child>.jsonl layout.
	if sessionID != "" && foreignParentID(cached.ParentConversationID, cached.ConversationID, sessionID) == "" {
		sessionsDir := agentlinks.SessionsDir(firstNonEmpty(probe.CWD, cached.CWD))
		if parent := resolveSubagentParent(sessionsDir, sessionID, vendor); parent != "" {
			cached.ParentConversationID = parent
			cached.IsBackgroundAgent = true
		}
	}
	// Top-level Cursor chats must never keep a recovered parent. Stale
	// sessionstate / greedy pending-claim previously stamped parent_id and
	// hid the chat from Sessions forever.
	if sessionID != "" && agentlinks.IsTopLevelCursorChat(sessionID) {
		if cached.ParentConversationID != "" || cached.IsBackgroundAgent {
			cached.ParentConversationID = ""
			cached.IsBackgroundAgent = false
			if sessionsDir := agentlinks.SessionsDir(firstNonEmpty(probe.CWD, cached.CWD)); sessionsDir != "" {
				_ = agentlinks.Unregister(sessionsDir, sessionID)
			}
		}
	}
	...
	maybeAuditApproval(vendor, event, cached.PermissionMode, sessionID, probe.CWD)

	// Guardrails deny (PreToolUse / shell / file) - write vendor JSON to stdout
	// and skip the rest of the telemetry path when denied.
	if maybeEnforceGuardrails(vendor, event, payload, probe.CWD) {
		return nil
	}

	cfg, err := agentconfig.Load(nil)
	if err != nil {
	...
	// user-set SUPEROPEN_APPLICATION_NAME (file or env) still wins;
	...
	// Inject shared .so/memory pack on SessionStart (multi-agent).
	cwd := probe.CWD
	if cwd == "" {
		cwd = cached.CWD
	}
...
func canonicalVendor(vendor string) string {
	...
	case "opencode", "open-code":
		return "opencode"
	case "copilot", "copilot-cli", "copilotcli":
		return "copilot-cli"
	case "pi":
		return "pi"
	default:
...
// resolveSubagentParent recovers parent chat id for a child Task /
// subagent conversation that did not carry parent_conversation_id.
func resolveSubagentParent(sessionsDir, sessionID, vendor string) string {
	if sessionsDir == "" || sessionID == "" {
		return ""
	}
	if p := agentlinks.ResolveParent(sessionsDir, sessionID, vendor); p != "" {
		return p
	}
	if p := agentlinks.ClaimPending(sessionsDir, sessionID, vendor); p != "" {
		return p
	}
	return ""
}
...
func logErrorf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "so hook: "+format+"\n", args...)
}
