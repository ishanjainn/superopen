...
// counters and prices USD from real counters via pricing.Lookup.
func TestCursorStopStampsRealTokenAttrs(t *testing.T) {
	...
	wantCost := pricing.Lookup("composer-2.5").Cost(500, 100, 200, 50)
	gotCost, ok := attrs["gen_ai.usage.cost"].(float64)
	if !ok || gotCost != wantCost {
		t.Errorf("gen_ai.usage.cost = %v, want %v", attrs["gen_ai.usage.cost"], wantCost)
	}
}

// TestCursorStopAccumulatesIntoSessionEnd verifies multiple stop
// turns sum into session-root token totals and priced CostUSD.
func TestCursorStopAccumulatesIntoSessionEnd(t *testing.T) {
	...
	wantCost := pricing.Lookup("composer-2.5").Cost(500, 100, 0, 0) + pricing.Lookup("composer-2.5").Cost(800, 200, 0, 0)
