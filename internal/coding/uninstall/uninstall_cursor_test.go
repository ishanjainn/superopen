...
func TestStripCursorHooks_RemovesSessionsFinalize(t *testing.T) {
  ...
  "hooks": {
    "sessionEnd": [
      {"command": "/tmp/so sessions finalize", "timeout": 30000},
      {"command": "other-tool cleanup"}
    ]
  }
}
`
	...
	got, err := stripCursorHooks(path, false)
	if err != nil {
		t.Fatal(err)
	}
	if got == "" {
	...
	if strings.Contains(string(raw), "sessions finalize") {
		t.Fatalf("finalize hook left: %s", raw)
	}
	if !strings.Contains(string(raw), "other-tool cleanup") {
