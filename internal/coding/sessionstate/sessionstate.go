...
// `flock_windows.go` uses LockFileEx for cross-process serialisation.
// Both implementations always invoke `fn` exactly once, regardless of
...
func GC(maxAge time.Duration) {
	...
	dir := filepath.Join(root, "so", "sessions")
