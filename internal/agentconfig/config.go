...
//  2. SUPEROPEN_* environment variables
//  3. OTEL_EXPORTER_OTLP_* environment variables (standard OTel fallback)
//  4. ~/.config/superopen/config.env on Unix, %APPDATA%\superopen\config.env
...
func builtinDefaults() Defaults {
	return Defaults{
		...
		// "metadata_only" via `so configure --content-capture`
		// (or SUPEROPEN_CODING_CONTENT_CAPTURE). The trace detail UI
...
//
// Secret-shaped keys (currently `SUPEROPEN_API_KEY`) are deliberately
...
func Load(flags *Flags) (*Resolved, error) {
	...
	// Step 2: SUPEROPEN_* env vars (override OTel ones).
	for _, k := range []string{
		"SUPEROPEN_OTLP_ENDPOINT",
		"SUPEROPEN_API_KEY",
		"SUPEROPEN_ENVIRONMENT",
...
// OTLP exporter and `so configure --show` agree on one truth.
