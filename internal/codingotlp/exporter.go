...
import (
	...
	sdk "github.com/superopen/so/sdk/go"
...
// the OTel SDK and writes the result back into `SUPEROPEN_USER`, so this
// function is normally a single env-var read.
//
// The OS-username fallback exists for direct CLI usage outside the
// hook hot path (e.g. `so configure`); it is the last resort and
...
func NewEmitter(_ context.Context, cfg *agentconfig.Resolved, vendor string, extraAttrs ...map[string]string) (*Emitter, error) {
	if cfg == nil {
	...
	// regular superopen-sdk SDK service. Without this, the SDK pipeline
...
// syncSOSessionState mirrors agent session lifecycle into .so/session-state/
// so `so status` / resume / git trailers see active sessions.
func syncSOSessionState(s normalize.Session) {
	if s.SessionID == "" {
		return
	}
	root := s.CWD
	...
	if !s.EndedAt.IsZero() || (s.Outcome != "" && s.Outcome != "started" && s.Outcome != "in_progress") {
		if !isSessionStart(s) {
	...
	head := ""
	if s.Extras != nil {
		branch = s.Extras["vcs.ref.head.name"]
		head = s.Extras["vcs.ref.head.revision"]
	}
