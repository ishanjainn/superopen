...
| Path | Role |
|------|------|
| `store.go` | SQLite persistence (schema_version=3, no ALTER migrate) |
...
| `embed.go`, `embedworker.go` | Optional embeddings; hashed fallback (one-time stderr if BGE never attaches) |
...
- Graph-owned channels (do not put memory playbooks here): `Block()`, `CursorRule()`, `SKILL.md` body, PreToolUse `graphGate` / `SearchNudge` / `ReadNudge` / `MemoryNudge` / `CaptureNudge`, Codex PreToolUse (must stay empty), UserPromptSubmit / PostToolUse / Stop. `Block()` / `CursorRule()` / `SKILL.md` may include **one** capture-vs-recall clause for live writes (intent, not a phrase list). File Read Gate on Read is out: graph owns Read; file memory is `so memory search --file`.
...
- SessionStart is a ≤350 token pointer: N memories + `so memory recall` as a Bash CLI invocation. It does not dump recency `#id` titles (there is no question yet). Code-shaped workspaces get a graph Bash one-liner (no memory encyclopedia). Memory-shaped workspaces MUST say N memories exist and give `so memory recall`. Do not stay silent just because Distill knowledge is empty. Empty store stays silent except a pending distill one-liner. Fail-open. Harvest OPEN review stays off SessionStart; a **pending** harvest/distill one-liner is allowed (live agent first).
...
- `so memory distill` is the only path that sends a session digest off-machine (opt-in via that session vendor's own one-shot CLI; live agents use `--brief` then `--apply`).
...
python3 benchmarks/tests/test_harness.py
```
