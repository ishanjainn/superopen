// Unity build: cgo only compiles .cpp files in this package directory.
...
static bool parse_line_directive(const char *line, size_t len, uint32_t *out_line, std::string *out_file) {
	size_t i = 0;
...
static bool build_line_map(const std::string &expanded, const std::string &main_file,
			   uint32_t *original_line_by_expanded_line, uint8_t *belongs_to_main_file) {
	...
	while (line_start <= expanded.size()) {
		...
		if (parse_line_directive(expanded.c_str() + line_start, line_end - line_start, &directive_line,
					 &directive_file)) {
...
SOPreprocessedSource *so_preprocess_with_map(const char *source, int source_len, const char *filename,
	...
	try {
		...
		SOPreprocessedSource *pp = (SOPreprocessedSource *)calloc(1, sizeof(*pp));
		if (!pp) {
			return NULL;
		}
		int line_count = count_expanded_lines(result);
		pp->source = (char *)malloc(result.size() + 1);
		pp->original_line_by_expanded_line = (uint32_t *)calloc((size_t)line_count + 1u, sizeof(uint32_t));
		...
		if (!pp->source || !pp->original_line_by_expanded_line || !pp->belongs_to_main_file) {
			so_preprocessed_source_free(pp);
			return NULL;
		}
		memcpy(pp->source, result.c_str(), result.size() + 1);
		if (!build_line_map(result, files[0], pp->original_line_by_expanded_line, pp->belongs_to_main_file)) {
			so_preprocessed_source_free(pp);
			return NULL;
		}
		return pp;
