...
func Leiden(nodes []int64, edges []LeidenEdge, resolution float64) []LeidenMembership {
	result := make([]LeidenMembership, len(nodes))
	for index, nodeID := range nodes {
		result[index] = LeidenMembership{NodeID: nodeID, Community: index}
	}
	if len(nodes) == 0 {
		return result
	}
...
func leidenWeights(nodes []int64, edges []LeidenEdge) ([]int, []int, []float64) {
